...
class MultiHeadAttention(nn.Module):
    ...
    def get_attn_probs(self, Q, K, V, mask=None):
        # the the linear transformation
        Q = self.split_heads(self.W_q(Q))
        K = self.split_heads(self.W_k(K))
        V = self.split_heads(self.W_v(V))

        # do everything except the output mapping for attn
        attn_scores = torch.matmul(Q, K.transpose(-2, -1)) / math.sqrt(self.d_k)
        if mask is not None:
            attn_scores = attn_scores.masked_fill(mask == 0, -1e9)
        attn_probs = torch.softmax(attn_scores, dim=-1)
...
# add a lightning module on top of it

class LitAutoEncoder(L.LightningModule):
    def __init__(self, transformer, tgt_vocab_size):
        super().__init__()
    ...
    def training_step(self, batch, batch_idx):
        ...
        # vocabulary size
        tgt_vocab_size = self.transformer.tgt_vocab_size
    ...
    def configure_optimizers(self):
        ...
        optimizer = torch.optim.Adam(self.transformer.parameters(), lr=0.0001, betas=(0.9, 0.98), eps=1e-9)
        return optimizer
